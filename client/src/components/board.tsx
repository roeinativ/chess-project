...
export default function Board({
  ...
  const [pieceColor, setPieceColor] = useState<"w" | "b">(
    color === "white" ? "w" : "b",
  );
  ...
  function onPieceDrop({ sourceSquare, targetSquare }: onPieceDropArgs) {
    ...
    if (isTurn) {
      ...
      const premoveOnSquare = preMovesRef.current.find(
        (p) => p.targetSquare === sourceSquare,
      );
      ...
      if (premoveOnSquare) {
        ...
        preMovesRef.current.push({
          ...
          targetSquare,
          piece: {
            pieceType: pieceOnSquare.color + pieceOnSquare.type.toUpperCase(),
          },
        });
      }
      ...
      {promotionMove && (
        ...
        >
          {(["q", "r", "n", "b"] as const).map((piece) => (
            ...
            >
              {defaultPieces[
                `${pieceColor}${piece.toUpperCase()}` as keyof PieceRenderObject
              ]()}
            </button>
          ))}
        </div>
