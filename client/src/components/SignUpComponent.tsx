...
export default function SignUpComponent({
  navSignIn, signUp, enterUsername, setEnterUsername, password, setPassword, errorMessage
}: Types.SignUpType) {
  ...
  return (
    <main className="min-h-screen w-full flex items-center justify-center bg-[#0a0a0a] px-4 relative overflow-hidden">
      ...
      <div className="auth-card relative z-10 w-full max-w-sm rounded-xl border border-[#c8a96e]/20 bg-white/[0.03] backdrop-blur-sm p-8 shadow-2xl">
        ...
        <h1 className="auth-title text-2xl font-bold text-white text-center mb-1">Create account</h1>
        <p className="text-white/40 text-sm text-center mb-8">Join and start playing chess</p>
        ...
        <form onSubmit={signUp} className="flex flex-col gap-5">
          ...
          {errorMessage !== "" && (
            <Alert variant="destructive" className="border-red-500/30 bg-red-500/10">
              ...
              <AlertTitle className="text-red-400">Sign up failed</AlertTitle>
        ...
        <p className="text-center text-white/30 text-sm mt-6">
          ...
          <button onClick={navSignIn} className="text-[#c8a96e] hover:text-[#d4ba85] transition-colors bg-transparent border-none cursor-pointer underline underline-offset-2">
            Sign in
          </button>
        </p>
      </div>
